...
export function getAddressInfo(chainName, env) {
    const result = RELAYER_CONTRACTS[env][chainName];
    if (!result)
        throw Error(`No address info for chain ${chainName} on ${env}`);
    return result;
}
export function getWormholeRelayerAddress(chainName, env) {
...
export function getWormholeRelayer(chainName, env, provider, wormholeRelayerAddress) {
...
export const getNameFromCCTPDomain = (domain, environment = "MAINNET") => {
    if (domain === 0)
