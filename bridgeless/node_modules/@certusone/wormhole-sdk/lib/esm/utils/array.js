...
import { CHAIN_ID_ALGORAND, CHAIN_ID_APTOS, CHAIN_ID_INJECTIVE, CHAIN_ID_NEAR, CHAIN_ID_OSMOSIS, CHAIN_ID_PYTHNET, CHAIN_ID_SOLANA, CHAIN_ID_SUI, CHAIN_ID_TERRA, CHAIN_ID_TERRA2, CHAIN_ID_WORMCHAIN, CHAIN_ID_UNSET, coalesceChainId, isEVMChain, isTerraChain, CHAIN_ID_XPLA, CHAIN_ID_SEI, CHAIN_ID_BTC, CHAIN_ID_COSMOSHUB, CHAIN_ID_EVMOS, CHAIN_ID_KUJIRA, CHAIN_ID_NEUTRON, CHAIN_ID_CELESTIA, CHAIN_ID_STARGAZE, CHAIN_ID_SEDA, CHAIN_ID_DYMENSION, } from "./consts";
 ...
 */
export const isHexNativeTerra = (h) => h.startsWith("01");
const isLikely20ByteCosmwasm = (h) => h.startsWith("000000000000000000000000");
export const nativeTerraHexToDenom = (h) => Buffer.from(stripZeros(hexToUint8Array(h.substr(2)))).toString("ascii");
export const uint8ArrayToHex = (a) => Buffer.from(a).toString("hex");
export const hexToUint8Array = (h) => {
    if (h.startsWith("0x"))
        h = h.slice(2);
 ...
 */
export const tryUint8ArrayToNative = (a, chain) => {
    ...
    if (isEVMChain(chainId)) {
        ...
        const h = uint8ArrayToHex(a);
        return humanAddress("wormhole", isLikely20ByteCosmwasm(h) ? a.slice(-20) : a);
    }
...
export const tryHexToNativeStringNear = (provider, tokenBridge, address) => __awaiter(void 0, void 0, void 0, function* () {
    const { found, value } = yield hashLookup(provider, tokenBridge, address);
    if (!found) {
 ...
 * @throws if address is not the right length for the given chain
 */
export const tryHexToNativeAssetString = (h, c) => c === CHAIN_ID_ALGORAND
 ...
 */
export const hexToNativeAssetString = (h, c) => {
    if (!h) {
        return undefined;
    }
    try {
 ...
 * @throws if address is not the right length for the given chain
 */
export const tryHexToNativeString = (h, c) => tryUint8ArrayToNative(hexToUint8Array(h), c);
/**
 *
 * Convert an address in a wormhole's 32-byte hex representation into a chain's native
 * string representation.
 ...
 * @deprecated since 0.3.0, use [[tryHexToNativeString]] instead.
 */
export const hexToNativeString = (h, c) => {
    if (!h) {
        return undefined;
    }
    try {
 ...
 */
export const tryNativeToHexString = (address, chain) => {
    const chainId = coalesceChainId(chain);
    if (isEVMChain(chainId)) {
 ...
 * @throws if address is a malformed string for the given chain id
 */
export const nativeToHexString = (address, chain) => {
    if (!address) {
        return null;
    }
 ...
 * @throws if address is a malformed string for the given chain id
 */
export const uint8ArrayToNative = (a, chainId) => hexToNativeString(uint8ArrayToHex(a), chainId);
