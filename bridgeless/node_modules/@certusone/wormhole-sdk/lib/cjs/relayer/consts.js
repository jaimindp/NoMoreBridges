...
exports.getNameFromCCTPDomain = exports.getWormscanAPI = exports.getCircleAPI = exports.GUARDIAN_RPC_HOSTS = exports.RPCS_BY_CHAIN = exports.getWormholeRelayer = exports.getWormholeRelayerAddress = exports.getAddressInfo = exports.RELAYER_CONTRACTS = void 0;
...
exports.RELAYER_CONTRACTS = { MAINNET, TESTNET, DEVNET };
function getAddressInfo(chainName, env) {
    const result = exports.RELAYER_CONTRACTS[env][chainName];
    if (!result)
        throw Error(`No address info for chain ${chainName} on ${env}`);
    return result;
}
exports.getAddressInfo = getAddressInfo;
function getWormholeRelayerAddress(chainName, env) {
    const result = getAddressInfo(chainName, env).wormholeRelayerAddress;
    if (!result)
        throw Error(`No Wormhole Relayer Address for chain ${chainName}, network ${env}`);
    return result;
}
exports.getWormholeRelayerAddress = getWormholeRelayerAddress;
function getWormholeRelayer(chainName, env, provider, wormholeRelayerAddress) {
    const thisChainsRelayer = wormholeRelayerAddress || getWormholeRelayerAddress(chainName, env);
    const contract = ethers_contracts_1.WormholeRelayer__factory.connect(thisChainsRelayer, provider);
    return contract;
}
exports.getWormholeRelayer = getWormholeRelayer;
exports.RPCS_BY_CHAIN = {
    MAINNET: {
...
exports.getWormscanAPI = getWormscanAPI;
const getNameFromCCTPDomain = (domain, environment = "MAINNET") => {
    if (domain === 0)
...
exports.getNameFromCCTPDomain = getNameFromCCTPDomain;
