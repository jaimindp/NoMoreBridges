...
class TestLib {
    ...
    uint8ArrayToHexString(arr, add0x) {
        const ret = Buffer.from(arr).toString("hex");
        if (!add0x) {
            return ret;
        }
        return "0x" + ret;
    ...
    genGFeePayout(signers, guardianSet, nonce, seq, amt, dest) {
        const b = [
            "0x",
            this.zeroBytes.slice(0, 28 * 2),
            this.encoder("uint8", this.ord("C")),
     ...
     */
    createSignedVAA(guardianSetIndex, signers, timestamp, nonce, emitterChainId, emitterAddress, sequence, consistencyLevel, payload) {
        const body = [
