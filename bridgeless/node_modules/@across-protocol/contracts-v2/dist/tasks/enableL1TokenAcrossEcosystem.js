...
(0, config_1.task)("enable-l1-token-across-ecosystem", "Enable a provided token across the entire ecosystem of supported chains")
    ...
    const hre = hre_;
    const matchedSymbol = Object.keys(constants_1.TOKEN_SYMBOLS_MAP).find((symbol) => symbol === taskArguments.token);
    (0, assert_1.default)(matchedSymbol !== undefined, `Could not find token with symbol ${taskArguments.token} in TOKEN_SYMBOLS_MAP`);
    const hubPoolChainId = await hre.getChainId();
    const l1Token = constants_1.TOKEN_SYMBOLS_MAP[matchedSymbol].addresses[hubPoolChainId];
    ...
    let inputChains = [];
    try {
        const parsedChains = taskArguments.chains.split(",");
        inputChains = parsedChains.map((x) => Number(x));
        console.log(`\nParsed 'chains' argument:`, inputChains);
    }
    catch (error) {
        throw new Error(`Failed to parse 'chains' argument ${taskArguments.chains} as a comma-separated list of numbers.`);
    }
    if (inputChains.length === 0)
        inputChains = enabledChainIds;
    ...
    const tokens = await Promise.all(chainIds.map((chainId) => {
    ...
    console.table(chainIds.map((chainId, index) => {
        return {
            chainId,
    ...
    if (!(await (0, utils_1.askYesNoQuestion)("\n2. Do these token addresses match your expectations?")))
        process.exit(0);
    ...
    chainIds.forEach((fromId, fromIndex) => {
        chainIds.forEach((toId, _) => {
            ...
            if (depositRouteChains.length === 0 ||
                ...
                console.log(`\t\t Skipping route ${fromId} -> ${toId} because it doesn't involve a chain on the exclusive list`);
            }
        });
    });
    ...
    if (depositRouteChains.length === 0) {
        ...
        chainIds.forEach((toId, toIndex) => {
            ...
            if (depositRouteChains.length === 0 || depositRouteChains.includes(toId)) {
                ...
                console.log(`\t\t Skipping pool rebalance rout -> ${toId} because it doesn't involve a chain on the exclusive list`);
            }
        });
    ...
    console.log(`CallData contains ${callData.length} transactions, which can be sent in one multicall to hub pool @ ${hubPoolDeployment.address}🚀`);
