    ...
    if (variableType.encoding === 'inplace') {
        ...
        for (const [varName, varVal] of Object.entries(variable)) {
            ...
            if (variableType.key.startsWith('t_uint')) {
                key = padNumHexSlotValue(ethers_1.BigNumber.from(varName).toHexString(), 0);
            }
            else if (variableType.key.startsWith('t_bytes')) {
                key = padBytesHexSlotValue('0x' + (0, utils_1.remove0x)(varName).padEnd(64, '0'), 0);
            }
            else {
                key = padBytesHexSlotValue(varName, 0);
            }
            const prevBaseSlotKey = baseSlotKey || padNumHexSlotValue(storageObj.slot, 0);
            const nextBaseSlotKey = ethers_1.ethers.utils.keccak256(key + (0, utils_1.remove0x)(prevBaseSlotKey));
...
async function getMappingTypeStorageSlots(storageLayout, variableName, vmManager, contractAddress, key, storageObjectType, mappingKey, baseSlotKey) {
    ...
    if (storageObjectType.key.startsWith('t_uint')) {
        mappKey = padNumHexSlotValue(ethers_1.BigNumber.from(mappingKey[0]).toHexString(), 0);
    }
    else if (storageObjectType.key.startsWith('t_bytes')) {
        mappKey = padBytesHexSlotValue('0x' + (0, utils_1.remove0x)(mappingKey[0]).padEnd(64, '0'), 0);
    }
    else {
        mappKey = padBytesHexSlotValue(mappingKey[0], 0);
    }
    const prevBaseSlotKey = baseSlotKey || key;
    let nextSlotKey = ethers_1.ethers.utils.keccak256(mappKey + (0, utils_1.remove0x)(prevBaseSlotKey));
    let slotKeysTypes = [];
    mappingKey.shift();
