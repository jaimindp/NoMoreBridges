...
function filterItem(item, fixed) {
    switch (item.binary) {
        ...
        case "uint": {
            const { custom } = item;
            const isFixedItem = (0, utils_1.isPrimitiveType)(custom) || (0, utils_1.isFixedPrimitiveConversion)(custom);
            return (fixed && isFixedItem || !fixed && !isFixedItem) ? item : null;
        }
        case "array": {
...
function internalAddFixedValuesItem(item, dynamicValue) {
    switch (item.binary) {
        ...
        case "uint": {
            ...
            return item?.omit
                ? undefined
                : (0, utils_1.isPrimitiveType)(custom)
                    ? custom
                    : (0, utils_1.isFixedPrimitiveConversion)(custom)
                        ? custom.to
                        : dynamicValue;
        }
        case "array":
