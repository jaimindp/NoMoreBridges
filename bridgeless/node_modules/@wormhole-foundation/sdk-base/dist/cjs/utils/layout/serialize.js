...
function serializeLayout(layout, data, encoded, offset = 0) {
    const cursor = { bytes: encoded ?? new Uint8Array((0, size_1.calcLayoutSize)(layout, data)), offset };
...
function serializeLayoutItem(item, data, cursor) {
    switch (item.binary) {
        ...
        case "uint": {
            const value = (() => {
                if ((0, utils_1.isNumType)(item.custom)) {
                    if (!("omit" in item && item.omit))
                        (0, utils_1.checkNumEquals)(item.custom, data);
                    return item.custom;
                }
                if ((0, utils_1.isNumType)(item?.custom?.from))
        ...
        case "bytes": {
            ...
            if ("layout" in item) {
                ...
                if ((0, utils_1.isBytesType)(custom)) {
                    if (!("omit" in item && item.omit))
                        (0, utils_1.checkBytesTypeEqual)(custom, data);
