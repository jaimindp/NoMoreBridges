...
exports.calcLayoutSize = exports.calcStaticLayoutSize = void 0;
    ...
    switch (item.binary) {
        ...
        case "bytes": {
            ...
            if ("layout" in item) {
                ...
                return lengthSize + (0, utils_1.checkItemSize)(item, layoutSize);
            }
            const { custom } = item;
            if ((0, utils_1.isBytesType)(custom))
                return lengthSize + custom.length; //assumed to equal item.size if it exists
            if ((0, utils_1.isFixedBytesConversion)(custom))
                return lengthSize + custom.from.length; //assumed to equal item.size if it exists
            if (custom === undefined)
                return data ? lengthSize + (0, utils_1.checkItemSize)(item, data.length) : null;
            return data !== undefined ? lengthSize + (0, utils_1.checkItemSize)(item, custom.from(data).length) : null;
        }
        case "array": {
...
function internalCalcLayoutSize(layout, data) {
    if ((0, utils_1.isLayoutItem)(layout))
        return calcItemSize(layout, data);
    let size = 0;
...
function calcStaticLayoutSize(layout) {
    return internalCalcLayoutSize(layout);
}
exports.calcStaticLayoutSize = calcStaticLayoutSize;
function calcLayoutSize(layout, data) {
    const size = internalCalcLayoutSize(layout, data);
    if (size === null)
        throw new Error(`coding error: couldn't calculate layout size for layout ${layout} with data ${data}`);
    return size;
}
