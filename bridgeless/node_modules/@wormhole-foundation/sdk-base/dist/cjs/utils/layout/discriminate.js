...
const lengthSizeMax = (lengthSize) => lengthSize > 0 ? (1 << (8 * lengthSize)) - 1 : Infinity;
function layoutItemMeta(item, offset, fixedBytes) {
    switch (item.binary) {
        case "int":
        case "uint": {
            const fixedVal = (0, utils_1.isNumType)(item.custom)
                ? item.custom
                : (0, utils_1.isNumType)(item?.custom?.from)
                    ? item.custom.from
                    : null;
            if (fixedVal !== null && offset !== null) {
                const cursor = { bytes: new Uint8Array(item.size), offset: 0 };
                (0, serialize_1.serializeNum)(fixedVal, item.size, cursor, item.endianness, item.binary === "int");
        ...
        case "bytes": {
            ...
            if ("layout" in item) {
                ...
                if (custom !== undefined && typeof custom.from !== "function") {
                    fixed = (0, serialize_1.getCachedSerializedFrom)(item);
                    fixedSize = fixed.length;
                }
                else {
                    const layoutSize = (0, size_1.calcStaticLayoutSize)(item.layout);
                    if (layoutSize !== null)
                ...
                else if ((0, utils_1.isFixedBytesConversion)(custom)) {
                    fixed = custom.from;
            ...
            if (lengthSize > 0 && offset !== null) {
                if (fixedSize !== undefined) {
                    ...
                    (0, serialize_1.serializeNum)(fixedSize, lengthSize, cursor, endianess, false);
                    fixedBytes.push([offset, cursor.bytes]);
                }
                offset += lengthSize;
            }
        ...
        case "switch": {
            ...
            const caseBounds = item.layouts.map(([idOrConversionId, layout], caseIndex) => {
                ...
                if (offset !== null) {
                    ...
                    (0, serialize_1.serializeNum)(idVal, idSize, cursor, idEndianness);
