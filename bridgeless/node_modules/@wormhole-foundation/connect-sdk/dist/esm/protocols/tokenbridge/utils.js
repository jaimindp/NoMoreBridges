...
import { TokenBridge, deserialize, isNative, isTokenTransferDetails, isTransactionIdentifier, isWormholeMessageId, serialize, toUniversal, } from "@wormhole-foundation/sdk-definitions";
import { DEFAULT_TASK_TIMEOUT } from "../../config";
import { ErrInvalidStateTransition, TransferState, isAttested, isSourceFinalized, isSourceInitiated, } from "../../types";
...
export const truncate = (amt, decimals) => decimals > 8 ? amt / 10n ** (decimals - 8n) : amt;
// scale by number of decimals, pair with truncate
export const scale = (amt, decimals) => decimals > 8 ? amt * 10n ** (decimals - 8n) : amt;
// dedust for source using the number of decimals on the source chain
// but scale back to original decimals
export const _dedust = (decimals) => (amt) => scale(truncate(amt, decimals), decimals);
// truncate and rescale to fit destination token
export const _mintable = (srcDecimals, dstDecimals) => (amt) => scale(truncate(amt, srcDecimals), dstDecimals);
export class TokenTransferUtils {
    ...
    static async isTransferComplete(toChain, receipt) {
        if (!isAttested(receipt))
            return false;
        const vaa = deserialize("TokenBridge:TransferWithPayload", serialize(receipt.attestation.attestation));
