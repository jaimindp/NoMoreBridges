...
class EvmTokenBridge {
    ...
    constructor(network, chain, provider, contracts) {
        ...
        this.chainId = connect_sdk_1.nativeChainIds.networkChainToNativeChainId.get(network, chain);
    ...
    async getOriginalAsset(token) {
        ...
        const [chain, address] = await Promise.all([
            tokenContract.chainId().then(Number).then(connect_sdk_1.toChainId).then(connect_sdk_1.toChain),
    ...
    async getWrappedAsset(token) {
        if ((0, connect_sdk_1.isNative)(token.address))
        ...
        return new connect_sdk_evm_1.EvmAddress(wrappedAddress);
    }
    ...
    async *transfer(sender, recipient, token, amount, payload) {
        ...
        if ((0, connect_sdk_1.isNative)(token)) {
            ...
            if (allowance < amount) {
                ...
                yield this.createUnsignedTx((0, connect_sdk_evm_1.addFrom)(txReq, senderAddr), 'TokenBridge.Approve');
            }
            const sharedParams = [
                tokenAddr,
                amount,
    ...
    async *redeem(sender, vaa, unwrapNative = true) {
        ...
        if (vaa.payloadName === 'TransferWithPayload' &&
            ...
            const fromAddr = new connect_sdk_evm_1.EvmAddress(vaa.payload.from).unwrap();
        ...
        const tokenAddr = new connect_sdk_evm_1.EvmAddress(vaa.payload.token.address).unwrap();
