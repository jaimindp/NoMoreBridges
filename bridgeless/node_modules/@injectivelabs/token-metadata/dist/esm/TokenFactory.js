...
import { getTokenFromMeta, getTokenFromMetaIncludingIbcBaseDenoms, } from './utils/token';
...
export class TokenFactory {
    ...
    static make(network = Network.Mainnet, registry = {}) {
        if (isTestnet(network)) {
            return new TokenFactory(new TokenMetaUtils({ ...getTokensBySymbolForTestnet(), ...registry }));
        }
        if (network === Network.Devnet) {
            return new TokenFactory(new TokenMetaUtils({ ...getTokensBySymbolForDevnet(), ...registry }));
        }
        if (network === Network.Devnet1) {
            return new TokenFactory(new TokenMetaUtils({ ...getTokensBySymbolForDevnet1(), ...registry }));
        }
        if (network === Network.Devnet2) {
            return new TokenFactory(new TokenMetaUtils({ ...getTokensBySymbolForDevnet2(), ...registry }));
        }
        return new TokenFactory(new TokenMetaUtils({ ...tokensBySymbol, ...registry }));
    }
    toToken(denom) {
