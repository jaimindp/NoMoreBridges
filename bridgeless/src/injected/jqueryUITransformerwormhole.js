...
import chainIdLogo from './configs/chainIdLogo.json';
...
async function onAccounts(accounts) {
    ...
    try {
        ...
        // const chainName = 'Arbitrum'; // or 'Ethereum'
        ...
        // const srcChain = wh.getChain('Goerli');
        ...
        class MySignAndSendSigner {
            ...
            async signAndSend(tx) {
                // sign and send the transactions using ethersSigner
                // return the transaction hashes
                tx = tx[0];
                console.log(tx);
                const txParams = {
                    from: await this.ethersSigner.getAddress(),
                    to: tx.transaction.to,
                    data: tx.transaction.data,
                    chainId: Number(tx.transaction.chainId),
                };
        ...
        let signer = await provider.getSigner(account);
        let mySigner = new MySignAndSendSigner(signer);
        ...
        //     parseAmount(1, srcChain.config.nativeTokenDecimals)
        // );
        ...
        // // 1) Submit the transactions to the source chain, passing a signer to sign any txns
        // const srcTxids = await usdcXfer.initiateTransfer(mySigner);
        ...
        // const destTxids = await usdcXfer.completeTransfer(mySigner);

        //         // OR for an automatic transfer
        // const automaticXfer = await wh.tokenTransfer(
        //     1, // send native gas on source chain
        //     amount * 1000000, // amount in base units
        //     senderAddress, // Sender address on source chain
        //     senderAddress, // Recipient address on destination chain
        //     true // Automatic transfer
        // );

        // console.log(automaticXfer);

        // // 1) Submit the transactions to the source chain, passing a signer to sign any txns
        // const srcTxids = await automaticXfer.initiateTransfer(mySigner);
        ...
        // const txids = await signSendWait(srcChain, txGenerator, mySigner);
        // console.log('Trans');
        // console.log(txids);
